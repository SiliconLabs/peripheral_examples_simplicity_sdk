# Peripheral Examples - VDAC Continuous Mode #

## Summary  ##

This project demonstrates the VDAC operating in continuous mode with a single‑ended output to generate a stable 0.5 V signal on a pin while the device is in EM3.  
Because the VDAC runs independently of the core, the output voltage remains stable even under load. Continuous operation is reflected in the device’s power consumption.

Note:
Approximate current consumption values were measured using Simplicity Studio Energy Profiler. All projects were built using the default import configuration: Debug (-g3) and no optimization (gcc -O0). Refer to device‑specific datasheets for detailed low‑energy mode current specifications.
...
|-----------|-----------------|-------------------------------------------------------|
| BRD4204D  | 32 µA           | 435 µA                                                |
| BRD4186C  | 32 µA           | 550 µA                                                |
| BRD4270B  | 36 µA           | 885 µA                                                |
| BRD4117A  | 36 µA           | 1770 µA                                               |
| BRD4400C  | 33 µA           | 685 µA                                                |
...
2. Use an oscilloscope to measure the VDAC output pin.
