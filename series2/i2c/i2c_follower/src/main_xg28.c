/***************************************************************************//**
 * @file main_xg28.c
 * @brief This project demonstrates the follower configuration of the EFx32xG2x
...
/***************************************************************************//**
 * @brief SYSRTC initialization
 ******************************************************************************/
void initSYSRTC(void)
{
  // Select LFRCO as source for SYSRTC clock branch
  CMU_ClockSelectSet(cmuClock_SYSRTCCLK, cmuSelect_LFRCO);

  // Enable SYSRTC clock
  CMU_ClockEnable(cmuClock_SYSRTC, true);

  // Setup SYSRTC parameters
  sl_sysrtc_config_t sysrtcConfig = SYSRTC_CONFIG_DEFAULT;

  // Initialize SYSRTC
  sl_sysrtc_reset();
  sl_sysrtc_init(&sysrtcConfig);
...
void initI2C(void)
{
  ...
  // Enable GPIO pins PA11 (SDA) and PA12 (SCL)
  GPIO_PinModeSet(gpioPortA, 11, gpioModeWiredAndPullUpFilter, 1);
  GPIO_PinModeSet(gpioPortA, 12, gpioModeWiredAndPullUpFilter, 1);

  // Route I2C pins to GPIO
  GPIO->I2CROUTE[0].SDAROUTE = (GPIO->I2CROUTE[0].SDAROUTE & ~_GPIO_I2C_SDAROUTE_MASK)
                        | (gpioPortA << _GPIO_I2C_SDAROUTE_PORT_SHIFT
                        | (11 << _GPIO_I2C_SDAROUTE_PIN_SHIFT));
  GPIO->I2CROUTE[0].SCLROUTE = (GPIO->I2CROUTE[0].SCLROUTE & ~_GPIO_I2C_SCLROUTE_MASK)
                        | (gpioPortA << _GPIO_I2C_SCLROUTE_PORT_SHIFT
                        | (12 << _GPIO_I2C_SCLROUTE_PIN_SHIFT));
