...
This project demonstates the PRS clear functionality of the watchdog. The GPIO
is configured as an input and expects to receive a periodic signal with a
frequency > 0.5 Hz. The GPIO is connected to a PRS producer, and the WDOG is
connected to a PRS consumer of the same PRS channel. If the GPIO stops
receiving the periodic signal, the watchdog will not be cleared and trigger a
reset. Out of reset, the system checks the reset cause. If the latest reset was
caused by a watchdog timeout, LED0 turns on and the code does not execute any
further.
