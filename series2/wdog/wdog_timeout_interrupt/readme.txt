wdog_timeout_interrupt

This project demonstrates the functionality of the watchdog timer interrupt. It
uses LED0 and a Push-button to indicate the state of the system. LED0 will
flash as long as Push Button 0 is not pressed and the watchdog has not timed
out. While Push Button 0 is pressed, LED0 will stop flashing. If Push Button 0
is pressed and held through the WDOG timeout period, the watchdog's ISR will
trigger. LED0 is turned on and the code does not execute any further.
...
4. LED is now on and not blinking anymore, which indicates a WDOG timeout
