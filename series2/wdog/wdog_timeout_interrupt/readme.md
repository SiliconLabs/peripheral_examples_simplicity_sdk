...
This project demonstrates the functionality of the WDOG timer interrupt. The WDOG is configured to trigger an interrupt if not fed within 2049 ms. The WDOG is fed every 100 ms and LED0 toggles each time the WDOG is fed. Pressing Push Button 1 (PB1) blocks the application from feeding the WDOG. If PB1 is pressed and held through the WDOG timeout period, the WDOG triggers an interrupt. LED0 turns on and the application stops running.
