wdog_window_interrupt

This project demonstrates the window interrupt functionality of the watchdog
timer. In this example, the watchdog triggers an interrupt when the watchdog is
cleared within 50% of the timer period. The timer period is set to 2048 ms, so
the watchdog must be cleared after 1024 ms. The main loop clears the watchdog
every 1100 ms. When Push Button 1 is pressed, the watchdog clears after 800 ms,
triggering the wdog window interrupt. LED0 is turned on and the code does not
