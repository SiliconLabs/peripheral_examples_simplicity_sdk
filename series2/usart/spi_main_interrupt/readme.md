# Peripheral Examples - SPI main with interrupts #
...
4. Before running the example, set a breakpoint on the sl_gpio_set_pin()
   function call that de-asserts the chip select in SEND_DONE state. Examine the 
   inbuf[] array to see the received data from the secondary. This example
