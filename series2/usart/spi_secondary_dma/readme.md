# Peripheral Examples - SPI secondary with DMA #
...
4. Before running the example, set a breakpoint on the beginning of case 
   RECEIVE_DONE state in app_process_action(). Examine the inbuf[] array to 
   see the received data from the secondary. The main example transfers 
   data continuously but will not start until PB0 is pressed.
