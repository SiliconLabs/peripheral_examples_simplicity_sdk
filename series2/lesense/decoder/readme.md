# Peripheral Examples - LESENSE Decoder #
...
![State machine image](image/state_machine.jpg)

> Note: In projects where the device enters EM2 or lower, an escape_hatch
...
1. Disable EM2 debugging feature (Search Power Manager in the slcp and press on the
   gear to open Power Manager Configuration)
...
only powers IOVDD and the serial flash. To measure the 3.3V LDO current, 
replace R201 with R202. The current provided below is the combination of the 
VMCU and 3.3V current.

Note for Testing on xG28 (BRD4400C):
There is a diversity SPDT switch on the 2.4 GHz radio output that
...
* CMU
  * LFRCO @ 32768 Hz
* ACMP
  * Used to sample push-button 0 and push-button 1 input state
* GPIO
  * LED0 and LED1 configured as push-pull output
* LESENSE
  * Controls ACMP to sample push-button 0/1 on selected LESENSE channel
...
4. Run the project, press PB0 and/or PB1, the `LESENSE_IRQHandler` should trigger
...
7. Continue running the example and try different push button configurations
