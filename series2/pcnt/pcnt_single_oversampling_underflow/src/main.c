 ...
 * counter goes below zero. In this example, each press of Push Button PB0 will
 * decrease the counter value by 1. The initial value of the counter and the
 * reload value for the top register is set by PCNTTopValue define.
 *******************************************************************************
 * # License
...
// PCNT Top value reloaded with underflow
#define PCNTTopValue    5    // Interrupt every 6 BTN0 presses
 ...
 *        Counter and top value set to PCNTTopValue defined above;
...
static void initPcnt(void)
{
   ...
   * will cause the counter to decrement without a button press.
   *
  ...
  /*
   * Decrementing below zero (zero is CNT default) automatically reloads top
   * value into the CNT register. Clear underflow interrupt flag before
   * enabling.
   *
   ...
   * desire is to decrement counter on push button release. Note that negating
   * both (negEdge and PRS logic) results in the same problem and would require
   * the underflow clear that has been implemented for accurate count of the
   * first underflow event.
   */
  PCNT_IntClear(PCNT0, PCNT_IEN_UF);
