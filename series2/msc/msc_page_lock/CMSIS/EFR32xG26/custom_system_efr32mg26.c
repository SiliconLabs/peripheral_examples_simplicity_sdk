...
/* Begin code added for 'msc_page_lock' example */
   ...
   * C runtime startup code (e.g. startup_gcc_efr32mg24), at which
   * this, or other such time-critical initialization tasks,  can be
   * undertaken.
   *
   ...
   * MSC_PAGELOCK1 below needs to be executed as the first
   * instruction in main().
   */
  CMU->CLKEN1_SET = CMU_CLKEN1_MSC;
  MSC->PAGELOCKWORD12 = LASTLOCK;

  /*
   * To see how soon this happens relative to writing to one of the
   * MSC registers in main(), drive the GPIO connected to LED0 on the
   * Wireless Starter Kit mainboard high.  The delay from when this
   * pin goes high to low is the amount of time spent executing the
   * C runtime start-up code.
   */
  CMU->CLKEN0_SET = CMU_CLKEN0_GPIO;

#if (BSP_GPIO_LED0_PORT > 7)
  GPIO->P_SET[BSP_GPIO_LED0_PORT].MODEH = _GPIO_P_MODEL_MODE0_PUSHPULL << (4 * BSP_GPIO_LED0_PIN);
#else
  GPIO->P_SET[BSP_GPIO_LED0_PORT].MODEL = _GPIO_P_MODEL_MODE0_PUSHPULL << (4 * BSP_GPIO_LED0_PIN);
#endif
