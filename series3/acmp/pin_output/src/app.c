...
void acmp_init(void)
{
  ...
  // Invert ACMP output to GPIO. When GPIO_PB0 is lower than
  // 1.25 V then the GPIO output is 1 and when GPIO_PB0 is higher than
  // 1.25 V then the GPIO output is 0.
  sl_hal_acmp_setup_gpio_inversion(ACMP0, 1);

  // Route ACMP output to LED0
  GPIO->ACMPROUTE[0].ACMPOUTROUTE = (GPIO_LED0.port << _GPIO_ACMP_ACMPOUTROUTE_PORT_SHIFT)
                                    | (GPIO_LED0.pin << _GPIO_ACMP_ACMPOUTROUTE_PIN_SHIFT);
  GPIO->ACMPROUTE[0].ROUTEEN = GPIO_ACMP_ROUTEEN_ACMPOUTPEN;
