...
crc_t soft_crc(crc_t message, crc_t preset)
{
...
void gpcrc_init (void)
{
  ...
  init.reverse_bits = true;  // Reverse all bits of the incoming message
...
void app_init(void)
{
  ...
  for (int i = 0; i < ARRAY_SIZE; i++)
  {
    ...
    softResults[i] = soft_crc(source[i], PRESET);

    // Feed in source
    sl_hal_gpcrc_write_input_32bit(GPCRC0, source[i]);

    // Read result
    results[i] = sl_hal_gpcrc_read_data_bit_reversed(GPCRC0);
  }
