 ...
 *   This function toggles PRS to provide the cycles to synchronize an externally
 *   clocked PCNT.
 *
 * @param[in] pcnt
...
void pcnt_wait_sync(PCNT_TypeDef *pcnt)
{
  ...
  while (pcnt->SYNCBUSY & _PCNT_SYNCBUSY_MASK) {
    // toggle prs channel to synchronize PCNT
...
void PCNT0_IRQHandler(void)
{
  ...
  sl_hal_pcnt_clear_interrupts(PCNT0, PCNT_IF_DIRCNG);

  if (PCNT0->STATUS & PCNT_STATUS_DIR)    {     // check direction for toggling LEDs
    sl_gpio_toggle_pin(&GPIO_LED0);             // LED0 if down
  } else {
    sl_gpio_toggle_pin(&GPIO_LED1);             // LED1 if up
  }
}
...
{
  sl_hal_pcnt_init_t init = SL_HAL_PCNT_INIT_DEFAULT(SL_HAL_PCNT_MODE_EXT_CLK_QUAD);
  ...
  sl_hal_pcnt_enable_interrupts(PCNT0, PCNT_IEN_DIRCNG);
