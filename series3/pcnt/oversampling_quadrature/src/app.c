...
// Filter is uint8_t, set to max
#define FILTER_LENGTH 0xFF
...
{
  uint32_t flags = sl_hal_pcnt_get_pending_interrupts(PCNT0);
  sl_hal_pcnt_clear_interrupts(PCNT0, flags);
...
{
  sl_hal_pcnt_init_t init = SL_HAL_PCNT_INIT_DEFAULT(SL_HAL_PCNT_MODE_OVS_QUAD_4X);
  ...
  sl_hal_prs_connect_channel_consumer(PCNT_PRS_Ch1,
                                      SL_HAL_PRS_TYPE_ASYNC,
                                      SL_HAL_PRS_CONSUMER_PCNT0_S1IN);

  init.s1_count_direction = false;
  init.s0_prs_enable = true;
  init.s1_prs_enable = true;
  init.filter_enable = true;
  init.filter_lenght = FILTER_LENGTH;
  init.hysteresis_enable = true;
  ...
  sl_hal_pcnt_enable_interrupts(PCNT0, (PCNT_IEN_OF | PCNT_IEN_UF | PCNT_IEN_DIRCNG));
 ...
 *        This function sets up GPIO PRS which links BTN0 to PCNT0 PRS0 and
 *        BTN1 to PCNT0 PRS1
 ******************************************************************************/
static void init_prs(void)
{
  ...
  sl_hal_prs_async_connect_channel_producer(PCNT_PRS_Ch1,
                                            SL_HAL_PRS_ASYNC_GPIO_PIN2);
}

/***************************************************************************//**
 * @brief Initialize GPIO
 *        This function initializes LED pins, push buttons PB0 and PB1 and
 *        enables external interrupts for PRS functionality
 ******************************************************************************/
static void init_gpio(void)
{
  int32_t button0_int_no = 1;
  int32_t button1_int_no = 2;
  sl_gpio_init();
