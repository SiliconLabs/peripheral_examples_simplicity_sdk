...
void eusart0_init(void)
{
   ...
   * be set, but this requires disabling the EUSART because the
...
void app_process_action(void)
{
   ...
   *
   * Data to be queued for transmission is sent with sl_hal_eusart_tx(),
   * which checks that there is room in the transmit FIFO (EUSART_STATUS_TXFL).
   *
   * Incoming data is read from the FIFO with sl_hal_eusart_rx(),
   * which polls EUSART_STATUS_RXFL to make sure data has arrived.
   */
  for (i = 0; i < BUFLEN; i++){
    sl_hal_eusart_tx(EUSART0, buffer[i]);
    buffer[i] = sl_hal_eusart_rx(EUSART0);
  }
}
