...
void init_ldma_scatter(void)
{
  ...
  for (i = 1; i < LIST_SIZE - 1; i++)
  {
    descriptor_link[i] = (sl_hal_ldma_descriptor_t)SL_HAL_LDMA_DESCRIPTOR_LINKREL_M2M(SL_HAL_LDMA_CTRL_SIZE_HALF,
                                                                               0,
                                                                               &dest_buffer[LAST_INDEX - i],
                                                                               BUFFER_SIZE,
                                                                               1);

    // Descriptor source is relative
    descriptor_link[i].xfer.src_addr_mode = SL_HAL_LDMA_CTRL_SRC_ADDR_MODE_REL;
  }
  ...
  descriptor_link[LIST_SIZE - 1].xfer.src_addr_mode = SL_HAL_LDMA_CTRL_SRC_ADDR_MODE_REL;
...
void init_ldma_gather(void)
{
  ...
  // Use memory transfer configuration macro
  sl_hal_ldma_transfer_config_t memory_transfer_tx = SL_HAL_LDMA_TRANSFER_CFG_MEMORY();
  ...
  for (i = 1; i < LIST_SIZE - 1; i++)
  {
    ...
    descriptor_link[i].xfer.dst_addr_mode = SL_HAL_LDMA_CTRL_SRC_ADDR_MODE_REL;
  }
  ...
  descriptor_link[LIST_SIZE - 1].xfer.dst_addr_mode = SL_HAL_LDMA_CTRL_SRC_ADDR_MODE_REL;
